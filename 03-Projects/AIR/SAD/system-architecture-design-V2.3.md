# AIR (Advice & Intelligence Relationship) System Architecture Design — V2.3

> **Version**: 2.3
> **Based on**: System Architecture Design V2.2 + ADR-036 (Audit Trail) + ADR-037 (UI Theming)
> **Date**: June 2026
...
System_Boundary(airSystem, "AIR Platform") {
    ...
    ' === Tier 2: Presentation ===
    Container(spa, "Angular SPA", "Angular 19+, TypeScript", "Single-page application providing customer workbench, lead & opportunity management, consumer advisory workspace, proposal builder, and dashboards. Supports tenant-aware theming — runtime brand skinning based on authenticated user's JWT claims (see ADR-037).")
    ...
    ContainerDb(db, "PostgreSQL Database", "AWS RDS PostgreSQL 15+", "Stores leads, advice cases, proposals, compliance artefacts, engagement records, notebook, audit trail. Includes dedicated audit schema for application interaction audit (see ADR-036).")
...
' === Cross-cutting services ===
    ...
    Component_Ext(auditService, "Application Audit Service", "Spring AOP + PostgreSQL", "Cross-cutting audit trail — captures user interactions, state transitions, data access events. Append-only, immutable, 7-year retention (see ADR-036)")
...
A cross-cutting audit service captures all significant user interactions across the AIR platform. The service is implemented via Spring AOP interceptors on controller/service boundaries, ensuring consistent capture without polluting domain logic.
...
**Auditable Action Categories:** Data access, state transitions, proposal lifecycle, compliance interactions, AI agent interactions, lead management, consent & documents, administrative actions.

---

### Tenant-Aware UI Theming (ADR-037)

The Angular SPA supports runtime brand skinning based on the authenticated user's tenant/brand affiliation. A single build artefact serves all brands — no per-tenant deployments.
...
- Theme resolved from JWT claims at authentication — no additional API calls
- CSS custom properties as the theming layer — runtime switching without recompilation
- Angular Material M3 design tokens consume the custom properties
- Feature toggles (ADR-014) combine with tenant context for per-brand feature visibility
- Terminology overrides via i18n/label service (e.g. "advisor" → "consultant" for IFA partners)
- Default theme (FNB internal) applied immediately; tenant overrides applied post-authentication
...
| Supplementary | Cross-Cutting Concerns | Documents application audit trail and tenant-aware UI theming architecture |
...
- **Application Interaction Audit Trail**: Cross-cutting audit service captures all significant user interactions, state transitions, and data access events. Append-only immutable records with canonical schema (who, what, when, where, outcome). Implemented via Spring AOP interceptors with async write path. Dedicated audit schema in PostgreSQL with 7-year retention (FAIS compliance). Supervisors and compliance officers access via dedicated reporting views. See ADR-036.
- **Tenant-Aware UI Theming**: Angular SPA supports runtime brand skinning based on authenticated user's tenant/brand affiliation (resolved from JWT claims). Single codebase/deployment serves multiple brands via CSS custom properties over Angular Material's M3 design token system. Enables internal FNB branding and future external IFA white-label presentations without separate builds. Theme configurations include colour palettes, logos, typography, and optional terminology overrides. Feature toggles combine with tenant context for per-brand feature visibility. See ADR-037.
...
| Application Audit Service | Cross-cutting service capturing all significant user interactions, state transitions, and data access events in an append-only immutable store for regulatory compliance (FAIS, FICA, POPIA) |
| Tenant-Aware Theming | Runtime UI skinning system that applies brand-specific visual identity (colours, logos, typography) based on the authenticated user's tenant claim, enabling multi-brand delivery from a single Angular SPA build |
| IFA | Independent Financial Advisor — external (non-FNB) advisor who may use AIR under a different brand/white-label presentation |

---

*Document version: 2.3 — System Architecture (Audit Trail + UI Theming)*
*Based on: System Architecture Design V2.2 + ADR-036 + ADR-037*
*Date: June 2026*
...
| Application Audit Service | Cross-cutting (all modules) | BIAN does not prescribe a unified application-level audit trail pattern; AIR implements this as a regulatory requirement (FAIS, FICA, POPIA) |
| Tenant-Aware UI Theming | Angular SPA (presentation) | BIAN does not model presentation-layer concerns; AIR requires multi-brand skinning for internal vs external advisor populations |
