...
Rel(leadOpportunityModule, eventBus, "Publishes LeadSignalDetected, LeadPromotedToQueue, LeadFeedbackRecorded, OpportunityReadyForEngagement, QueueRanked")
Rel(advisoryServicesModule, eventBus, "Publishes AdviceCaseCreated, StageAdvanced, CaseCompleted")
Rel(proposalModule, eventBus, "Publishes ProposalSectionEdited, ProposalCompleted")
Rel(sessionDialogueModule, eventBus, "Publishes EngagementCaptured, EngagementSummaryAvailable")
Rel(documentServicesModule, eventBus, "Publishes ClientAccepted, DocumentGenerated")
Rel(complianceModule, eventBus, "Publishes ValidationCompleted")
...
Rel(eventBus, advisoryServicesModule, "Subscribes to OpportunityReadyForEngagement, ValidationCompleted, ClientAccepted")
Rel(eventBus, complianceModule, "Subscribes to ProposalCompleted (sync validation gate)")
...
Shows the physical deployment topology on AWS. Unchanged from V1.0 — deployment is infrastructure-level and independent of domain naming.
...
Deployment_Node(aws, "AWS Cloud", "Amazon Web Services") {
    ...
    Deployment_Node(s3_node, "S3 Bucket", "Static Hosting") {
        Container(angular, "Angular SPA", "Angular 19+ build artefacts", "Customer workbench, dashboards, proposal builder UI")
    }
